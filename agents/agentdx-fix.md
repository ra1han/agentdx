...
- Add or document a project-native lifecycle command surface for relevant common tasks: `bootstrap`, `doctor`, `boot`, `health`, `fast`, `proof`, `ci-equivalent`, `smoke`, `seed/reset`, `observe`, and `cleanup`
...
- Make command names predictable and agent-friendly, using names like `bootstrap`, `doctor`, `boot`, `health`, `fast`, `proof`, `smoke`, `seed`, `reset`, `observe`, and `cleanup` where the task runner supports them
